...
public class GUIold extends JFrame {
	...
	private Dimension textareasize = new Dimension(170, 300);
    ...
    private Dimension chatscrollsize = new Dimension(200,350);
	...
	public GUIold(int pcnumber, MultiCast2 multiCast) {
		 ...
		 */
		chats = new JPanel(new BorderLayout());
        JPanel availablechats = new JPanel(new BorderLayout());
        availablechats.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.black)));
        chats.add(availablechats, BorderLayout.CENTER);
        JPanel newchatbut = new JPanel();
        newchatbut.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.black)));
        chats.add(newchatbut, BorderLayout.SOUTH);
...
//        newchat.setPreferredSize(new Dimension(80, 30));
		newchat.addActionListener(new NewChatButton());
        newchatbut.add(newchat);
//        newchatgbc.anchor = GridBagConstraints.LAST_LINE_END;
//        newchatgbc.weighty = 1.0;
//        newchatgbc.weightx = 1.0;
//        newchatgbc.fill = GridBagConstraints.NONE;
        chatscrollpane = new JScrollPane(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
//        chatscroll.setPreferredSize(chatscrollsize);
//        chatbuttonsgbc.weightx = 2.0;
//        chatbuttonsgbc.weighty = 1.0;
//        chatbuttonsgbc.fill = GridBagConstraints.NONE;
//        chatbuttonsgbc.anchor = GridBagConstraints.FIRST_LINE_END;
        availablechats.add(chatscrollpane, BorderLayout.CENTER);
        chatscroll = new JPanel();
        chatscroll.setPreferredSize(chatscrollsize);
        chatscroll.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.GREEN), "Available Chats"));
        chatscrollpane.add(chatscroll);
//		chats.add(newchat,newchatgbc);
		chatLobbyPanel.add(chats, BorderLayout.CENTER);
        chatLobbyPanel.add(eastside, BorderLayout.EAST);
        eastside.setPreferredSize(chatwindowsize);
        eastside.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.black), "Messages"));
		setResizable(false);
	}
	...
	private class NewChatButton implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			JOptionPane.showMessageDialog(GUIold.this, newchatoptions, "New Chat Options", JOptionPane.PLAIN_MESSAGE);
            ...
            chatwindow.repaint();
            chatwindow.scrollmessages.revalidate();
            chatwindow.scrollmessages.repaint();
...
//				multiCast.send("chat" + chatbutton.name + ":" + textfield.getText(), chatwindow.participants[i]);
	...
	public class ChatWindow extends JPanel {
		...
		public int[] participants = new int[4];
        public JScrollPane scrollmessages;
		...
		public ChatWindow(int[] participants) {
			...
			scrollmessages = new JScrollPane(messages, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
	...
	public void printMessage(String message, int pc) {
		...
		if (message.contains("joinrequest:chat")) {
			...
			JOptionPane.showConfirmDialog(GUIold.this, name + " added you to chat " + chatnumber, "New Chat", JOptionPane.OK_OPTION);
